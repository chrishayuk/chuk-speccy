...
/// A stripped, decode-once executor for **straight-line** cells over the opcode subset the
/// compiler emits — the fast path for [`Runner::run_many_fast`]. It skips everything the
/// authentic `step` does for hardware fidelity but a headless cell never observes:
/// per-byte opcode fetch + contention, the R refresh register, the Q-flag quirk, interrupt
/// bookkeeping, flag computation, and per-cycle ticking. [`decode`] returns `None` (→
/// authentic fallback) on any branch/call/`halt`/shift or unknown opcode, so it never has
/// to be a *correct full* Z80 — only correct for the straight-line arithmetic it accepts,
/// and that is checked against the authentic interpreter by the differential tests.
mod fast {
    use super::SP_TOP;
    use crate::ORG;

    /// A decoded instruction — only the arithmetic opcode subset straight-line scoring
    /// cells emit. `rr`: 0=BC, 1=DE, 2=HL. `r` (8-bit): 0=B,1=C,2=D,3=E,4=H,5=L,7=A
    ...
    pub(super) fn decode(code: &[u8], entry: u16) -> Option<Vec<Op>> {
        ...
        loop {
            let op = *code.get(pc)?;
            // LD r,r' — 8-bit register moves, excluding any `(HL)` form (index 6) and HALT.
            if (0x40..=0x7F).contains(&op) && op != 0x76 {
                let (dst, src) = ((op >> 3) & 7, op & 7);
                if dst == 6 || src == 6 {
                    return None;
                }
                ops.push(Op::LdReg(dst, src));
                pc += 1;
                last_a = None;
                continue;
            }
            ...
            match op {
                0x01 => {
                    ops.push(Op::Ld16Imm(0, rd16(code, pc + 1)?));
                    adv = 3;
                }
                0x21 => {
                    ops.push(Op::Ld16Imm(2, rd16(code, pc + 1)?));
                    adv = 3;
                }
                0x2A => {
                    ...
                    match *code.get(pc + 1)? {
                        ...
                        0xFE => {
                            match last_a? {
                                ...
                                0x20 => ops.push(Op::Fill),
                                _ => return None, // HALT (0x30) / unknown → fall back
                            }
                            adv = 2;
                        }
                        _ => return None,
